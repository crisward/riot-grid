...
riot.tag2('gridbody', '<div onscroll="{scrolling}" riot-style="height:{parseInt(parent.opts.height,10)-30}px" class="gridbody"> <div riot-style="position:relative;height:{rowheight*parent.opts.data.length}px;background:white" class="scrollblock"> <div each="{row, i in visibleRows}" riot-style="top:{parent.rowheight*(i+parent.scrollTop)}px" ondblclick="{handleDblClick}" onclick="{handleClick}" class="gridrow {active:parent.active==row}"><yield></yield></div> </div> </div>', '', '', function(opts) {
this.active = false;
...
this.handleClick = (function(_this) {
  return function(e) {
    ...
    return _this.parent.opts.onselect(e.item.row);
  };
})(this);
...
this.handleDblClick = (function(_this) {
  return function(e) {
    ...
    _this.active = e.item.row;
    if ((_this.parent.opts.onedit != null) && typeof _this.parent.opts.onedit === "function") {
      return _this.parent.opts.onedit(e.item.row);
    }
  };
})(this);
}, '{ }');